...
class pResNet(nn.Module):
    def __init__(self, depth, alpha, num_classes, n_bands, spatial_size, inplanes, bottleneck=False):
        super(pResNet, self).__init__()
        if spatial_size < 9:
            self.avgpool_size = 1
        elif spatial_size <= 11:
            self.avgpool_size = 2
        elif spatial_size <= 15:
            self.avgpool_size = 3
        elif spatial_size <= 19:
            self.avgpool_size = 4
        elif spatial_size == 21:
            self.avgpool_size = 5
        elif spatial_size == 27:
            self.avgpool_size = 6
        elif spatial_size == 29:
            self.avgpool_size = 7
        else:
        ...
        self.avgpool = nn.AvgPool2d(self.avgpool_size)
