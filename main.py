...
epochs = 200
nums_ResUnit = 3  # 残差单元总数
alpha = 48  # alpha因子
inplanes = 38  # 初始维数
spatial_size = 11  # 邻域像素块
train_samples = 200  # 训练样本
...
def start(group_log_dir, random_state, logger):
    ...
    model = DS_pResNet(nums_ResUnit, alpha, num_classes, n_bands, spatial_size, inplanes)
    if use_cuda:
        model = model.cuda()
